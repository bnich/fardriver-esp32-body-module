...
| **E4** | 1+2 | Mersen **`FEB-11-11`** + 2× **`FSB1`** boots | 📦 | **58.49** | ✅ Paid 09-10 (DK 2378-FEB-11-11-ND) + 2× `FSB1` boots ordered, ⬜ price. Verified **600 VAC/DC**, 10×38, free-hanging inline, crimp, UL Recognized, 100 kA SCCR, 3 captive O-rings, 155 °C, tulip clips. ⚠️ **Boots are required** — the non-breakaway holder ships without them (*"non-breakaway units do not include insulator boots"*); the O-rings seal the fuse compartment, the boots seal the terminations. ⬜ **Also tape-wrap** — Mersen: *"due to varying wire insulation sizes it is suggested that tape wrap be utilized for best results."* On 12 AWG, tape or a light adhesive-shrink over the boot. ⚠️ **Terminal `11` = Cu crimp #8–#12 → 12 AWG minimum on a 0.62 A tap** — buy wire and crimp tool to match. Not `FEB-11-11-BA` ($128.19, utility breakaway, *"pole knockdown"*) |
| **E5** | 1 | TDK **`HAQ-10T`** heatsink | 📦 | **25.66** | ✅ Paid 09-10. ◐ **Fit ~85% confident, not primary-sourced:** DigiKey's *"HEAT SINK FOR CNA30-100"* is a tagline, not an exclusivity claim; TDK lists HAQ-10T as related for the **CN-B110** series incl. **CN150B110**, and its **57.9 × 36.8 mm** is the standard quarter-brick footprint. 7.5 °C/W, 25 mm transverse fins, screw mount. TDK's sites 403 from this machine. ⬜ **`cn-b_e.pdf` (in Downloads) settles the fit, the `/CO` derating and the baseplate question in one read** |
| **E6** | 2 | Schurter **`FAC 0031.3803`** holders | 📦 | **9.78** | ✅ Paid 09-10 (2 @ $4.89, DK 486-1257-ND). DigiKey lists **400 V/16 A** vs the 600 VAC/VDC UL figure — IEC/VDE vs UL ratings; 400 V is ample for 84 V. 1 used (DC-DC #2, with E11); the `KLKD002` upstream covers DC-DC #1, so the 2nd is a spare |
| **E7** | 2 | Würth **`7448022010`** choke | 📦 | **17.84** | ✅ Paid 09-10 (2 @ $8.92, DK 732-13589-ND). ⚠️ **Two, not one:** TDK requires a choke per supply input, before the bulk cap; Cincon's EC7BW meets EN 50155 *"WITH EXTERNAL FILTER"* only. 10 mH, 2 A @ 70 °C, AEC-Q200, at 29% of rating |
| **E8** | 2 | Chemi-Con **`EKXJ221ELL221MM25S`** (KXJ, 220 µF / **220 V**) | 📦 | **8.02** | ✅ Paid 09-10 (2 @ $4.01, DK 565-EKXJ221ELL221MM25S-ND — *"CAP ALUM 220UF 20% 220V RADIAL"*). Meets the plan's ≥220 µF / 200 V requirement with margin; same `MM25S` 18 × 25 mm size as the 200 V `EKXJ201…`. ⚠️ **Two are needed, different nodes** — TDK's EMC bulk at DC-DC #1's input, hold-up behind the diode at #2; **440 µF total downstream of D13, which is what the SOA figure turns on** |
...
| **G2** | 2 | **`AO3407A`** P-channel MOSFET, −30 V, **±20 V gate** | ⬜ | ~1 | Q1, the hardware brake-lamp switch (plan D23), + 1 spare. SOT-23 — same adapter as D3 (X7 ②). A ±12 V-gate part is marginal on the 12 V rail |
...
| **H2** | 2 | **`BSS126`** N-channel, 600 V, SOT-23 | ⬜ | ~1.5 | Pulls Q3's gate down from the latch; 1 spare. ⚠️ A 30 V part (`AO3400A`) will not stand off the 84 V node |
...
| **X6** | 20 | SMD→DIP adapters, **SOIC-8** | 📦 | — | ✅ Ordered 09-10. ⚠️ **Fits nothing on this BOM** — keep as stock. X7 is what's needed |
| **X7** | 20 | **SMD→DIP adapters that fit** | ⬜ | ~30 | **① HTSSOP-28 + exposed pad** for D1 ×4 → Chip Quik / Proto Advantage **`2200235`** (*"TSSOP-28-Exp-Pad to DIP-28, 0.65 mm"* — the **Exp-Pad** variant is the one that matters); ⬜ **confirm it routes the pad to a pin or via array, or the thermal path is lost** (their site 403s from here). **Buy 4** — a failed mount can take the adapter. **② SOT-23-3** for D3's `AO3400A` ×10 and G2's `AO3407A` ×2 → Artekit **`DA-SOT23-3-X2`** or SchmalzTech **`ST-SOT23-3`** (DigiKey). **③ SOT-23-6 / TSOP6** for C2 ×4 → SparkFun **`00717`** ($1.25) or SchmalzTech **`ST-SOT23-6`** ($1.99) — Nexperia: *"SOT457 (also referred to as SOT23-6)"* |
...
| **09-10** | **DigiKey** — adapters | **X6** SMD→DIP SOIC-8 ×20 | ⬜ | 📦 ⚠️ Fits nothing on this BOM — keep as stock; **X7** is what's needed |
| **09-10** | **DigiKey — 10 lines** (SO 101547984) | **C2** ×20 · **C3** ×4 · **D1** ×4 · **D3** ×10 · **E2** · **E3** · **E5** · **E6** ×2 · **E7** ×2 · **E8** ×2 | **$182.96** <br>+ $8.49 ship · $14.68 tax · **$26.09 tariff** = **$232.22** | ✅ Submitted |
| **09-10** | **Mouser** | **E1** TDK `CN150B110-12/CO` | **$127.88** <br>+ **$31.97 tariff** | ⏳ Pending. 25% tariff |
| **09-10** | **DigiKey — fuse pair** | **E11** `0001.2504` ×5 · **E4** `FEB-11-11` | **$63.42** | ✅ Submitted |
| **09-10** | **Boots** | **E4** `FSB1` ×2 | ⬜ | ✅ Ordered — ⚠️ utility channel, not DigiKey |
| **09-11** | — | **A3** bar-mount switch sets ×2 | ⬜ | ◐ **LEFT received, harnessed, working.** ⏳ Right pod in transit — it has the lighting slider |

**Committed: $374.26** (+$31.97 tariff on the Mouser line). ⬜ Prices still to log: **X6**, **E4**'s boots, **A3**.
