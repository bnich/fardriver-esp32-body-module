...
decision log (D1–D24); its header carries the critical path. `docs/bom.md` **owns procurement** —
change a part there first, then the plan section that line's Notes names.
...
- ⛔ **`BSS126` is DEPLETION-mode — on at V<sub>GS</sub> = 0.** Wherever a 600 V small-signal N-FET is
  ...
  depletion FET as a pull-down switch holds the thing it controls ON with the key off.
...
- **TVS arrays follow the line's idle voltage:** `PESD5V0S4UD` (5 V) only on 3.3 V-class lines; 12 V
  lines and the brake-lever nodes (~11.4 V) take `SMS15T1G`. A 5 V array on a 12 V line is a short.
