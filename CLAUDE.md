...
pcb-layout-tools v0.10.0 (`pcbl`, installed on its own) lays the boards out. The gate checks all
...
constraints read by `pcbl stack` (pcb-layout-tools v0.10.0, which the gate requires by version) — and `tools/README.md` has one line per tool. Its step 0
...
- **The generated PCBs carry only the board-wide 0.2 mm** (JLC's capability template), and
  ...
  **pcb-layout-tools** (`pcbl`, tag v0.10.0); this repo describes the design to it through
