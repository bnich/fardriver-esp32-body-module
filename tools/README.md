...
`~/tools/lcsc-search`, an editor-saved `.eprj2` template and **pcb-layout-tools v0.9.0** (`pcbl` on
...
| `layout_export.py` | Describes the design to **pcb-layout-tools** (`pcbl`, tag v0.9.0), which places, routes and checks the boards: `python3 -m tools.layout_export PROJECT.eprj2 -o layout.yaml`. Every figure is read from where it already lives in this repo (📄 `layout/PROCESS.md`) |
