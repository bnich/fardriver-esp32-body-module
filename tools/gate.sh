...
# (`pcbl`, v0.7.0) accepts the layout constraints this repo writes for it.
#
...
PCBL_NEEDS="pcbl 0.7.0"
PCBL=${PCBL:-pcbl}
command -v "$PCBL" >/dev/null 2>&1 \
    || fail pcbl "not found (looked for '$PCBL'); the layout is checked with pcb-layout-tools v0.7.0 -- install that tag and put 'pcbl' on PATH, or set PCBL to it"
PCBL_HAS=$("$PCBL" --version 2>&1 || true)
[[ "$PCBL_HAS" == "$PCBL_NEEDS" ]] \
    || fail pcbl "this repo needs pcb-layout-tools v0.7.0 ('$PCBL_NEEDS'); '$PCBL --version' says '$PCBL_HAS'"
