...
is the owner's layout and the build **refuses to overwrite it**: bare, it exits `INCOMPLETE` and says
so; with `--keep-saved-layout` (which `tools/gate.sh` passes) it leaves the file untouched and counts it
as the project to open. A
...
|---|---|
| No layout yet | run the build; open the `.eprj2` it writes; Import Changes on each of the **four** PCBs; **flip the six bottom parts**; save |
...
**This directory** holds this README, `PROCESS.md` (how the boards are laid out, with pcb-layout-tools)
and `POWER-drc-exceptions.md` (the editor DRC hits on POWER that IO-29 accepts, generated by `pcbl`). The `.eprj2` is not in the repo: it is a binary the editor
