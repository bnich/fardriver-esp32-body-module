...
Placement, routing and the checks are done by **`pcb-layout-tools`** (`pcbl`, tag **v0.7.0**). The
...
refuses with a clear message when `pcbl` v0.7.0 is not installed. `pcbl check` is the layout's own
