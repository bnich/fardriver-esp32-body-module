...
tools/route.py  --heavy                  # R2: the copper the rules decide, + the DRC exceptions
...
|---|---|---|---|---|
| **HV** | `layout_rules.hv_nets(d, board)` — 14 on POWER | 0.5 mm | **1.25 mm to low-voltage copper; between two HV nets, their own voltage difference** (IO-29) | IPC-2221B B2 for the 160 V do-not-exceed. The clearance is `layout_rules.HV_CLEARANCE_MM` itself, the same figure check 7 uses as a placement proxy — not a second copy. The width is a FLOOR, not a current: the HV nets carry 1.93 A at the 60 V LVC, which 0.5 mm takes easily |
...
⭐ **The HV clearance is a PAIR's figure, and the tool is the authority on it (IO-29).** IPC-2221B
Table 6-1's row is *voltage between conductors*, so 1.25 mm is what pack voltage keeps from
**low-voltage** copper, and between two 84 V nets the figure is their own difference on the same B2
column — 0.1 mm at 0–30 V, 0.6 mm to 150 V, 1.25 mm above. `route.pair_clearance(a, b)` answers it,
`--heavy` routes to it and `--check-routing` checks to it, so the router and the DRC cannot
disagree. The **node voltages are derived, never typed**: `soft_start.hv_node_voltages` walks the
netlist for each node — the switch, both chokes, the blocking diode, and each divider string's own
resistor values — and the pair's figure is the widest difference the two reach at any of 24
operating points (pack 43 · 60 · 84 · 160 V, key on, key off, and the hold-up's ride-out, at both
ends of the rectifier's drop). ⚠️ **The key-off states are the harsh ones**, not an afterthought:
parked with the XT90-S mated, `HV_BPLUS` stands the whole pack against everything downstream of
Q101, so those pairs keep 1.25 mm. What relaxes is the gate network referred to the source —
`D13_GATE` to `HV_BPLUS` is D102's 15 V clamp, 0.1 mm — and nets that swing together, `HV_SW` to
`KSW`, 0 V. ⛔ `HV_C1_N` / `HV_C2_N` sit at 0 V DC and are **not** thereby low-voltage copper: the
choke winding that makes them 0 V DC is what puts the section's switching and surge swing on them,
so to anything outside the class they keep the full figure.

⚠️ **The editor cannot be told this**, which is why it is written out instead. EasyEDA Pro 3.2.149
binds a clearance to a net, not to a pair, and `--rules` deliberately leaves the HV rule at 1.25 mm
to everything so the autorouter never puts low-voltage copper near pack voltage. The joins its DRC
will then flag and IO-29 accepts are listed, one line each with the measured gap, the voltage
between the two nets and the IPC figure it meets, in **`layout/POWER-drc-exceptions.md`** —
generated by `--heavy` on every run. ⛔ **A DRC hit that is not on that list is a defect.**
...
1. **The 84 V chain on POWER** (HV). `J101.B+ → FH201 → Q101 → HV_SW → L101`/`L102` → `C201`/`C202`
   ...
   1.25 mm from low-voltage copper and at each pair's own voltage difference between two 84 V nets
   (IO-29), inside the region the placement drew, on one layer. ✔ **Before leaving it, run the
   editor's DRC with the HV rule on, and check every hit against
   `layout/POWER-drc-exceptions.md`.**
...
- ⭐ **The clearance was read off the wrong row, and IO-29 fixed the rule — but it is not what is
  refusing these legs.** The HV class now holds two 84 V nets to their own voltage difference (R0
  above), and the figures the refusals quote move with it: `D13_GATE` beside `HV_BPLUS` asks
  0.10 mm, `D13_GATE` beside `D13_PD_MID` 0.60. ⚠️ **The count did not move: 23 before, 23 after**,
  because the binding obstacle in 21 of them is a **0.00 mm** overlap — the straight run or its one
  L lies *across* a neighbouring pad, and no clearance figure reaches that. What the pairwise rule
  did buy is two necks the 84 V chain no longer needs and a POWER route that is 54 segments rather
  than 56. The remaining 23, measured:
  · **fifteen** cannot be drawn at any clearance (a pad in the path at 0.00 mm) — `C107.2 → D101.K`
    has to cross `C107.1`, `D102.A → R101A.1` has to cross `R101A.2`;
  · **five** miss a figure the physics really does ask for, by 0.02–0.35 mm: `HV_SW` to
    `HV_C2_HOLD_IN` at 1.22 mm against 1.25 (the ride-out: D201 blocks, so the hold-up node stands
    a whole pack against a collapsed `HV_SW`), `HV_C1_N` to `HV_C1_P` at 0.90 against 1.25 (the two
    sides of C201), `HV_C2_HOLD` to `U202.+Vout` at 1.09 against 1.25 (pack voltage against the 5 V
    rail), `HV_C2_N` to `KEY_SENSE_MID` at 0.51 against 0.60;
  · **three** are HV copper across a pin that is on **no net** (`U202.Remote`, `U202.Trim`) or on
    `GND` — a floating pin of a 160 V brick keeps the full figure, and that is right.
  ⬜ **So POWER's 84 V end is still not routed end to end**, and the reason is the placement and the
  tool's straight-or-one-corner repertoire, not the rule. The owner draws the rest by hand, or the
  gate network's parts move — a `--stack --keep` question, not a constant to loosen.
...
- **Clearance** — edge to edge, **what that PAIR needs** (`route.required_clearance`): the stronger
  of the two nets' class figures, except that between two HV nets it is their own voltage
  difference on IPC-2221B B2 (IO-29, R0 above). The same rule `--heavy` routes to, so the router
  and the DRC cannot disagree, and the message names the pair's figure and the volts behind it
  rather than the class's. Segment to segment, segment to pad, and a via to anything — a via's
  barrel crosses every layer, so it is the one object compared against copper on all of them.
  ⛔ Pad to pad is not checked: two pads of a land pattern sit where the manufacturer put them, and
  the editor's own `OTHER.deviceClearance` is 0 for the same reason. ⚠️ The **exception list** does
  look at pad to pad, because the editor's HV rule does — `--rules` raises every cell of its
  matrix, so an 0603 whose two pads are on different 84 V nets is a hit there. ⚠️ A pad is measured
  by its **copper** — `defaultPad`, with an equal-axis `ELLIPSE` treated as a circle — and not by
  the courtyard box `place.Envelope` carries.
...
⚠️ **On POWER, "DRC clean" means clean against `layout/POWER-drc-exceptions.md`** (IO-29). The
editor's HV rule is 1.25 mm to everything because its rules bind to a net and not to a pair, so it
flags joins the pairwise figure allows; that file lists every one of them with its measured gap,
the voltage between the two nets and the IPC row it meets. ⛔ **A hit that is not on the list is a
defect** — it goes back to `--check-routing` and to the placement, never onto the list by hand.
The file is regenerated by `--heavy` on every run, so a route that moves moves it too.
...
- `tools/soft_start.py` — **where the 84 V section's node voltages come from** (IO-29). It already
  owned Q101's gate network, so the walk that gives every 84 V node its DC voltage lives there and
  `route.py` only takes differences. ⛔ A new 84 V net with no derived voltage makes `--heavy` and
  `--check-routing` **refuse**, rather than quietly getting a relaxed figure.
- `layout/POWER-drc-exceptions.md` — **generated** by `--heavy`, rewritten on every run, never
  hand-edited: the joins the editor's DRC will flag and IO-29 accepts. One line per pair, with the
  gap, the volts between the two nets and the IPC-2221B B2 figure it meets.
