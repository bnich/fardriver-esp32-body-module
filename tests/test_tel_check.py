...
def design(extra_nets=()):
    parts = (Part("C1", "c", "0805", "CONV", "C", ("1", "2"), 1, v_max=50),
             Part("C2", "c", "r", "CONV", "C", ("+", "-"), 1, v_max=25),
             Part("C3", "c", "r", "CONV", "C", ("+", "-"), 1, v_max=25),
             Part("U1", "m", "m", "CONV", "CONVERTER", ("-Vin", "+Vout"), 1),
             Part("FH1", "clip", "clip", "CONV", "FUSECLIP", ("1",), 1),
             Part("FH2", "clip", "clip", "CONV", "FUSECLIP", ("1",), 1),
             Part("F1", "fuse", "5x20", "CONV", "FUSE", ("1", "2"), 1, v_max=300))
...
def test_a_matching_export_passes_with_the_fuse_expected_absent():
    ...
    r = tel_check.compare(design(), "CONV", tel_check.parse(TEL))
...
def test_a_pin_on_the_wrong_net_is_reported():
    ...
    r = tel_check.compare(design(), "CONV", tel_check.parse(bad))
    assert not r.ok
...
def test_a_missing_net_and_a_part_that_should_be_off_the_pcb_are_reported():
    ...
    r = tel_check.compare(design(), "CONV", tel_check.parse(with_fuse))
